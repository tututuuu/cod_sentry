...
//	
//  /* USB Send data */
//  //CDC_Transmit_FS((uint8_t *)SendPacket,MINIPC_SENDLENGTH);
//}

void MiniPC_SendFrameInfo(MiniPC_SendPacket_Typedef *SendPacket)
{
//  /* calculate the crc16 */
  SendPacket->checksum = get_CRC16_check_sum((uint8_t *)SendPacket,sizeof(MiniPC_SendPacket)-2,0xffff);

 /* store the MiniPC data to be sent */
  memcpy(MiniPC_SendBuf,(uint8_t *)SendPacket,sizeof(MiniPC_SendPacket));

  /* USB Send data */
  CDC_Transmit_FS(MiniPC_SendBuf,sizeof(MiniPC_SendPacket));
